...
namespace Coursework
{
    public partial class Form1 : Form
    {
        ...
        public class Editor
        {
            ...
            public Document GetActiveDocument()    // Возвращает активный документ через привязку вкладки и документа через свойство Tag.
            {
        ...
        {
            // Получаем текущий документ из активной вкладки
            var doc = _editor.GetActiveDocument();
            if (doc != null)
            {
                if (colorDialog1.ShowDialog() == DialogResult.OK)
                {
                    // Применяем цвет только к RichTextBox текущей вкладки
                    doc.TextBox.BackColor = colorDialog1.Color;
                }
            }
            else
            {
                MessageBox.Show("Нет активной вкладки для изменения фона", "Ошибка",
        ...
        private void fontSettingsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (doc != null)
            {
                ...
                {
                    // Применяем шрифт только к RichTextBox текущей вкладки
                    doc.TextBox.Font = fontDialog1.Font;
                }
            }
            else
            {
                MessageBox.Show("Нет активной вкладки для изменения шрифта", "Ошибка",
                                MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    }
